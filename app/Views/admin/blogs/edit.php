...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-3xl p-6 lg:p-8 border border-[#EDEDED] shadow-2xs">
        ...
        <div class="flex items-center justify-between pb-6 mb-6 border-b border-[#EDEDED]">
            <div>
                <h3 class="font-display text-[20px] font-bold text-[#252525]">Edit Article</h3>
                <p class="text-[13px] text-[#6F6F6F] mt-0.5">Update content, image, or publishing status</p>
            </div>
        ...
        <form action="<?= base_url('admin/blogs/update/' . $blog['id']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?= csrf_field() ?>
                ...
                <div>
                    ...
                    >
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= esc($cat) ?>" <?= old('category', $blog['category']) === $cat ? 'selected' : '' ?>><?= esc($cat) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-4 mb-3">
                    <div class="w-24 h-16 rounded-xl overflow-hidden bg-[#FCEAF2] border border-[#EDEDED] shrink-0">
                        <img src="<?= esc($blog['img']) ?>" alt="Current image" class="w-full h-full object-cover" />
                    </div>
                    <span class="text-[12.5px] text-[#6F6F6F]">Current featured thumbnail</span>
                </div>
                <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <span class="text-[12px] text-[#6F6F6F] block mb-1">Or Replace with new file</span>
                        <input
                            type="file"
                            name="img_file"
                            accept="image/*"
            ...
            <div class="flex flex-wrap items-center gap-6 pt-4 border-t border-[#EDEDED]">
                <label class="flex items-center gap-2.5 cursor-pointer">
                    ...
                    />
                    <span class="text-[13.5px] font-bold text-[#252525] font-display">Published (Visible on site)</span>
                </label>
